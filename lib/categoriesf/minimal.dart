...
class MinimalExtended extends StatefulWidget {
  static const String id = 'MinimalExtended';
  @override
  _MinimalExtendedState createState() => _MinimalExtendedState();
}

class _MinimalExtendedState extends State<MinimalExtended> {

  StreamSubscription<QuerySnapshot> subscription;
  List<DocumentSnapshot>popularList;
  final CollectionReference collectionReference = 
  Firestore.instance.collection("minimal");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body:SingleChildScrollView(
          ...
          children: <Widget>[
            SizedBox(height:80 ,),
            Text("Minimal",style: TextStyle(color: Colors.white,fontSize: 30,fontWeight: FontWeight.bold),),
            Container(
                ...
                padding: EdgeInsets.only(top:20,bottom:30 ),
                  ...
                  gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, crossAxisSpacing: 5, mainAxisSpacing: 10),
                         itemBuilder: (context,i) {
                            String mimgpath = popularList[i].data['url'];
                            return GestureDetector(
                             onTap: (){
                                        Navigator.of(context).push(  new MaterialPageRoute(
                                         builder: (BuildContext context) => MinimalFull(
                                                   mimgpath2: mimgpath,
...
class MinimalFull extends StatelessWidget {

  _save() async {
    var response = await Dio().get(mimgpath2, options: Options(responseType: ResponseType.bytes));
    final result = await ImageGallerySaver.saveImage(Uint8List.fromList(response.data));
    print(result);
  }
  static const String id = 'MinimalFull';
  MinimalFull({this.mimgpath2});
  String mimgpath2;
  @override
  Widget build(BuildContext context) {
    return  Scaffold(
        ...
        width:MediaQuery.of(context).size.height,
          child: CachedNetworkImage(
            placeholder: (context, url) => Center(child: new CircularProgressIndicator()),
            imageUrl: mimgpath2,fit: BoxFit.cover,),),
