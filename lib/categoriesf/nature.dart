...
class NatureExtended extends StatefulWidget {
  static const String id = 'NatureExtended';
  @override
  _NatureExtendedState createState() => _NatureExtendedState();
}

class _NatureExtendedState extends State<NatureExtended> {

  StreamSubscription<QuerySnapshot> subscription;
  List<DocumentSnapshot>popularList;
  final CollectionReference collectionReference = 
  Firestore.instance.collection("nature");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body:SingleChildScrollView(
          ...
          children: <Widget>[
            SizedBox(height:80 ,),
            Text("Nature",style: TextStyle(color: Colors.white,fontSize: 30,fontWeight: FontWeight.bold),),
            Container(
                ...
                padding: EdgeInsets.only(top:20,bottom:30 ),
                  ...
                  gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, crossAxisSpacing: 5, mainAxisSpacing: 10),
                         itemBuilder: (context,i) {
                            String nnimgpath = popularList[i].data['url'];
                            return GestureDetector(
                             onTap: (){
                                        Navigator.of(context).push(  new MaterialPageRoute(
                                         builder: (BuildContext context) => NatureFull(
                                                   nnimgpath2: nnimgpath,
...
class NatureFull extends StatelessWidget {

  _save() async {
    var response = await Dio().get(nnimgpath2, options: Options(responseType: ResponseType.bytes));
    final result = await ImageGallerySaver.saveImage(Uint8List.fromList(response.data));
    print(result);
  }
  static const String id = 'MinimalFull';
  NatureFull({this.nnimgpath2});
  String nnimgpath2;
  @override
  Widget build(BuildContext context) {
    return  Scaffold(
        ...
        width:MediaQuery.of(context).size.height,
          child: CachedNetworkImage(
            placeholder: (context, url) => Center(child: new CircularProgressIndicator()),
            imageUrl: nnimgpath2,fit: BoxFit.cover,),),
